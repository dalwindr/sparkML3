  ...
  def CallNaiveBayesAlgo(pipedDF: org.apache.spark.sql.DataFrame , pipedtestDF : org.apache.spark.sql.DataFrame, dataSetType:String )={
   ...
   import org.apache.spark.ml.classification.NaiveBayes
    ...
    if (dataSetType == "MultiClass")
         ...
         Confusion_matrixCalculation(nv_prediction,"General Random Forest Classfier")
         BinaryClassificationEvaluator_ROC(nv_prediction,"General Random Forest Classfier")
         BinaryClassificationEvaluator_PR(nv_prediction,"General Random Forest Classfier")
     ...
     println("--------------------------------------------------------------Applying General Decision tree Classfier ------------------------------------------")
      dt_prediction.groupBy(col("prediction"), col("label")).count().orderBy(col("count").desc_nulls_first).show(500)      
       if (dataSetType == "MultiClass")
             ...
             //Lets mearsure the accruarcy
             Confusion_matrixCalculation(dt_prediction,"General Random Forest Classfier")
             BinaryClassificationEvaluator_ROC(dt_prediction,"General Random Forest Classfier")
             BinaryClassificationEvaluator_PR(dt_prediction,"General Random Forest Classfier")
  ...
  def CallLogisticRegressionAlgo(pipedDF: org.apache.spark.sql.DataFrame , pipedtestDF : org.apache.spark.sql.DataFrame, dataSetType: String )={
       ...
       //create random forest model
        ...
        if (dataSetType == "MultiClass")
             ...
             //Lets mearsure the accruarcy
             Confusion_matrixCalculation(lr_prediction,"General Random Forest Classfier")
             BinaryClassificationEvaluator_ROC(lr_prediction,"General Random Forest Classfier")
             BinaryClassificationEvaluator_PR(lr_prediction,"General Random Forest Classfier")
 
          }                  
      }
  
def getStringIndexersArray(CatalogicalFeatureList:Seq[String]) ={
        CatalogicalFeatureList.map { colName =>new StringIndexer().setInputCol(colName).setOutputCol(colName + "Indexed").setHandleInvalid("skip")}.toArray
}
...
def CategoricalFeatureVectorzing( CatalogicalFeatureList:Seq[String]) = {  
                   ...
                   //Array of String indexes of string features
                   val stringIndexers = getStringIndexersArray(CatalogicalFeatureList)
                   val OHEncoder = CatalogicalFeatureList.map { colName =>new OneHotEncoderEstimator().setInputCols(Array(colName + "Indexed")).setHandleInvalid("skip").
                                                         setOutputCols(Array(colName + "Vector"))}.toArray
                   //val partial_Stage= 
